...
void Process(const Mat& in, Mat& out, const vector<Point> &points, vector<size_t> &convex_hull) {
  out = in.clone();
  ...
  for (size_t i = 0; i < convex_hull.size(); ++i) {
    ...
    stringstream s;
    s << i;
    putText(out, s.str(), points[point_start], CV_FONT_HERSHEY_COMPLEX, 1., Scalar(0, 255, 0));
  }
}

void ProcessWithFilter(const Mat& in, Mat& out, const vector<Point> &points, vector<size_t> &convexHull) {
...
int main(int argc, const char** argv) {
  ...
  bool has_camera = parser.has("camera");
  int camera_id = -1;
  if (has_camera)
  ...
  if (has_camera) {
    try {
      capture = unique_ptr<VideoCapture>(new VideoCapture(camera_id));
    } catch(...) {
      cout << "Wrong camera id or camera is disconnected" << endl;
      return 0;
    }
  } else {
    capture = unique_ptr<VideoCapture>(new VideoCapture(video_path));
  }
  ...
  EventData data;
  setMouseCallback(kWindowName, MyMouseCallback, &data);
  ...
  *capture >> frame;
  ...
  while (!frame.empty()) {
    ...
    for (size_t i = 0; i < data.points_position.size(); ++i)
      UpdatePointPosition(data.points_position[i], data.points_direction[i], frame_size);
